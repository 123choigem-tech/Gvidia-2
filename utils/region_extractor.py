...
# 동명이지(예: '고성' — 경남/강원)는 삽입 순서에 따라 한쪽 좌표가 선점되어
# 반대쪽 기사가 엉뚱한 좌표로 배정되므로, 서로 다른 좌표를 가리키는
# 모호한 별칭은 매핑에서 제외한다(미해결로 보고됨).
...
for alias, key in _ALIAS_INDEX:
    ...
    if alias in COASTAL_COORDS and COASTAL_COORDS[alias] != coord:
        _ambiguous_aliases.add(alias)
    else:
        COASTAL_COORDS.setdefault(alias, coord)
for _alias in _ambiguous_aliases:
    COASTAL_COORDS.pop(_alias, None)
...
def load_disaster_events() -> pd.DataFrame:
    ...
    # docstring이 약속한 대로 파일이 없으면 샘플 데이터로 폴백한다.
    print(f"[region_extractor] {_EVENTS_PATH} not found -> using sample events ({len(_SAMPLE_EVENTS)}건)")
    return pd.DataFrame(_SAMPLE_EVENTS)
