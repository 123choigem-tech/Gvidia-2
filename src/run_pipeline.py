...
def main():
    src_dir = sys.argv[1] if len(sys.argv) > 1 else os.path.join(_REPO_ROOT, "data", "input", "khoa_sst")
    hot_dir = sys.argv[2] if len(sys.argv) > 2 else os.path.join(_REPO_ROOT, "data", "results", "sst_analysis", "sst_over28")
    persist_dir = sys.argv[3] if len(sys.argv) > 3 else os.path.join(_REPO_ROOT, "data", "results", "sst_analysis", "persistence")
