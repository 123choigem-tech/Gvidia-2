...
DEFAULT_SRC_DIR = os.path.join(_REPO_ROOT, "data", "input", "khoa_sst")
DEFAULT_OUT_DIR = os.path.join(_REPO_ROOT, "data", "results", "sst_analysis", "timeseries")
...
# ---------------------------------------------------------------------------
def compute_daily_means(src_dir: str = DEFAULT_SRC_DIR, pattern: str = "*.nc",
...
def plot_timeseries(df: pd.DataFrame, out_png: str,
    ...
    try:
        return _draw_timeseries(fig, ax, d, out_png, title, show_max, ymin, ymax)
    finally:
        plt.close(fig)                      # 예외 시에도 figure 누수 방지


def _draw_timeseries(fig, ax, d, out_png, title, show_max, ymin, ymax):
    import matplotlib.dates as mdates
...
# ---------------------------------------------------------------------------
def run(src_dir: str = DEFAULT_SRC_DIR, out_dir: str = DEFAULT_OUT_DIR,
