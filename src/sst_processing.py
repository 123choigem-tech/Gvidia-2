...
def open_sst(path: str):
    ...
    #     (마스크/QA 변수를 잘못 잡을 수 있으므로 반드시 경고를 남긴다)
    for name, var in ds.data_vars.items():
        if var.ndim >= 2:
            print(f"  [경고] 알려진 SST 변수명이 없어 '{name}' 을 SST 로 간주합니다"
                  f" — 온도 변수가 맞는지 확인하세요: {os.path.basename(path)}")
            return ds, name
...
# ---------------------------------------------------------------------------
def process_file(in_path: str, out_dir: str = os.path.join(_REPO_ROOT, "data", "results", "sst_analysis", "sst_over28"),
                 threshold: float = HIGH_SST_THRESHOLD,
    ...
    try:
        ...
        # 원본이 아니라 튐값 제거(clean_sst) 후 배열에서 최댓값을 계산
        #  (hot_min/hot_max 와 동일 기준 — 99℃ 센서 오류 등이 요약에 섞이지 않게)
        _clean_vals = clean_sst(ds[sst_name]).values
        max_sst = float(np.nanmax(_clean_vals)) if np.isfinite(_clean_vals).any() else float("nan")
...
# ---------------------------------------------------------------------------
def process_dir(in_dir: str, out_dir: str = os.path.join(_REPO_ROOT, "data", "results", "sst_analysis", "sst_over28"),
                threshold: float = HIGH_SST_THRESHOLD,
...
if __name__ == "__main__":
    ...
    out_arg = sys.argv[2] if len(sys.argv) > 2 else os.path.join(_REPO_ROOT, "data", "results", "sst_analysis", "sst_over28")
