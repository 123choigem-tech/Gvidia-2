                ...
                with open(tmp, "wb") as f:
                    for chunk in r.iter_content(chunk_size=CHUNK):
                        if chunk:
                            f.write(chunk)
                            size += len(chunk)
            # 다 받은 뒤 검증: 크기(Content-Length 대조) + 매직바이트
            #   서버가 연결을 조기 종료해도 예외가 안 날 수 있어, 수신 바이트와
            #   선언된 크기가 다르면 절단 파일로 보고 재시도한다.
            if expected is not None and size != int(expected):
                print(f"  [경고]   {name} 수신 크기 불일치 {size} != {expected} (시도 {attempt}/{RETRIES})")
                if os.path.exists(tmp):
                    os.remove(tmp)
            elif is_valid_netcdf(tmp):
