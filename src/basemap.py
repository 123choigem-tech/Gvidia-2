...
# CWD가 아니라 저장소 루트 기준 절대경로 (어디서 실행해도 같은 위치)
_REPO_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DEFAULT_NE_DIR = os.path.join(_REPO_ROOT, "data", "naturalearth")
...
# 절단/오류 응답이 정상 파일로 남는 것을 막기 위한 최소 크기(바이트)
# (실측: shp ≈ 7.2MB, shx 188B, dbf 350B — 헤더 크기 이상만 요구)
_MIN_SIZE = {"shp": 1_000_000, "shx": 100, "dbf": 100}
...
def ensure_naturalearth(ne_dir: str = DEFAULT_NE_DIR, name: str = LAND_NAME) -> str:
    ...
    다운로드는 .part 임시 파일로 받고 크기 검증 후에만 최종 파일로 승격한다
    (손상 다운로드가 검증 없이 영구 잔존하던 문제 방지).
    """
    ...
    if need:
        ...
        for ext in need:
            ...
            dst = os.path.join(ne_dir, f"{name}.{ext}")
            tmp = dst + ".part"
            try:
                r = requests.get(url, timeout=120)
                r.raise_for_status()
                expected = r.headers.get("Content-Length")
                if expected is not None and len(r.content) != int(expected):
                    raise IOError(f"수신 크기 불일치: {len(r.content)} != {expected}")
                if len(r.content) < _MIN_SIZE.get(ext, 1_000):
                    raise IOError(f"파일이 비정상적으로 작음: {len(r.content)}B")
                ...
                if os.path.exists(tmp):
                    os.remove(tmp)
                raise RuntimeError(
                    f"Natural Earth 해안선({name}.{ext}) 다운로드 실패: {exc}\n"
                    f"오프라인 환경이면 {url} 을 수동으로 받아 {ne_dir}/ 에 두세요."
                ) from exc
    return shp
