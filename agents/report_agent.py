...
# 고정 파일명 대신 실제 존재하는 최신 시계열 PNG 사용
_ts_pngs     = sorted(SST_TS_DIR.glob("SST_daily_mean_*.png")) if SST_TS_DIR.exists() else []
SST_TS_PNG   = _ts_pngs[-1] if _ts_pngs else SST_TS_DIR / "SST_daily_mean_20250701_20250831.png"
...
_period_cache: tuple[str, str] | None = None


def _analysis_period() -> tuple[str, str]:
    """지역 SST CSV의 실제 날짜 범위에서 분석 기간을 산출 (하드코딩 제거)."""
    global _period_cache
    if _period_cache is None:
        from utils.sst_stats import iter_region_csvs

        lo, hi = None, None
        for _, df in iter_region_csvs(SST_DIR):
        ...
        if lo is not None and hi is not None:
            _period_cache = (lo.strftime("%Y-%m-%d"), hi.strftime("%Y-%m-%d"))
        else:
            _period_cache = ("2025-06-01", "2025-08-31")
...
def _load_sst_frames(threshold: float) -> tuple[dict, pd.DataFrame]:
    """지역별 SST CSV 로드 + 통계 DataFrame 반환 (공용 utils.sst_stats 사용)."""
    from utils.sst_stats import hot_stats, iter_region_csvs

    frames, stats = {}, []
    for region, df in iter_region_csvs(SST_DIR):
        frames[region] = df
        s = hot_stats(df, threshold)
        stats.append({
            "지역": region,
            "평균SST(°C)": round(s["avg"], 2) if s["avg"] is not None else None,
            "최고SST(°C)": round(s["max"], 2) if s["max"] is not None else None,
            f"고수온일수(≥{threshold}°C)": s["hot_freq"],
            "최장연속(일)": s["max_consec"],
        })
...
def _build_docx(
    ...
    _p0, _p1 = _analysis_period()
    doc.add_paragraph(f"분석 기간: {_p0} ~ {_p1}  |  고수온 기준: {threshold}°C")
    doc.add_paragraph()
    ...
    # 2페이지: 분석 이미지 3개
    doc.add_heading(f"위성 SST 공간·시계열 분석 ({_p0} ~ {_p1})", 1)
...
def _build_pdf(
    ...
    story += [
        ...
        Paragraph(
            ...
            f"해수면온도 {_analysis_period()[0]} ~ {_analysis_period()[1]} / 고수온 기준 <b>{threshold}°C</b>.", kor),
        Spacer(1, 4*mm),
    ]
    # 통계 표 먼저
    if freq_df is not None and not freq_df.empty:
