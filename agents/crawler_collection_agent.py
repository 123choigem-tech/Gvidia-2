...
공개 함수:
  crawl_articles(keyword, limit) -> list[dict]             # 라이브 크롤 (공개 진입점)
...
def crawl_articles(keyword: str, limit: int) -> list[dict]:
    """뉴스 크롤만 수행하는 공개 진입점 (pages/5_Pipeline.py 등에서 사용)."""
    return _live_crawl(keyword, limit)


# ---------------------------------------------------------------------------
# 3) 미해결 지역(위경도 None/NaN) 필터링 — collection 크래시 방지
#    (좌표 유효성 검사는 collection_agent._has_coord 를 재사용해 중복 제거)
# ---------------------------------------------------------------------------
from agents.collection_agent import _has_coord as _is_valid_coord  # noqa: E402
