"""
메인 에이전트 — CLI 전용 파이프라인 조율 (standalone)

순서: news_agent → collection_agent → alert_agent → report_agent

주의: Streamlit 앱의 파이프라인(pages/5_Pipeline.py)은 이 모듈을 쓰지 않고
crawler_collection_agent + scripts/collect_sst_by_region 경로를 사용한다.
이 모듈은 `python -m agents.main_agent` 로 직접 실행하는 CLI 진입점이다.
"""
