...
HOT_IMG_DIR = RESULTS_DIR / "sst_analysis" / "sst_over28" / "img"


@st.cache_data(ttl=300)
def load_region_freq() -> pd.DataFrame:
    freq_path = RESULTS_DIR / "frequency" / "region_frequency.csv"
...
def load_sst_by_region() -> dict[str, pd.DataFrame]:
    return load_region_data(config.DATA_DIR)


calc_hot_stats = hot_stats
...
with tab3:
    ...
    ss_imgs = sorted((RESULTS_DIR / "HS28NLS26" / "south_sea").glob("*_region.png"))
